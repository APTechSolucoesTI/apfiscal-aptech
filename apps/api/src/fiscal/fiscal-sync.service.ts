...
export class FiscalSyncService {
  ...
  private async config(companyId: string): Promise<IntegrationConfig> {
    const result = await supabaseAdmin
      ...
      .select(
        "primary_provider, fallback_provider, fallback_enabled, organization_id, ativo, certificate_storage_path, certificate_password_encrypted, api_key_encrypted",
      )
  ...
  async test(companyId: string, requestedProvider?: NfeProviderKind) {
    ...
    if (!providerConfigured(config, kind))
      throw new ProviderPreparationError(missingProviderMessage(kind));
  ...
  async sync(companyId: string) {
    ...
    const lock = await supabaseAdmin.rpc("try_acquire_fiscal_sync_lock", {
      _company_id: companyId,
      _worker_id: workerId,
    });
    if (lock.error) throw lock.error;
    if (!lock.data)
      throw new ConflictException("Já existe uma sincronização em andamento para esta empresa.");
    ...
    try {
      const [config, stateResult] = await Promise.all([
        ...
        supabaseAdmin
          ...
          .select("company_id, cnpj, last_nsu, next_allowed_sync_at")
          .eq("company_id", companyId)
          .single(),
      ]);
      ...
      const nextAllowed = stateResult.data.next_allowed_sync_at
        ? new Date(stateResult.data.next_allowed_sync_at)
        : null;
      if (nextAllowed && nextAllowed > new Date()) {
        throw cooldownException("A SEFAZ", nextAllowed);
      }
      ...
      const update = await supabaseAdmin
        ...
        .update({
          ...
          next_allowed_sync_at: retryAt?.toISOString() ?? null,
      ...
      await supabaseAdmin
        .from("empresa_integracoes_fiscais")
        .update({ ultimo_nsu: Number(result.lastNsu) })
        .eq("company_id", companyId);
      ...
      if (auditOrganizationId) {
        await supabaseAdmin.from("historico_integracao_fiscal").insert({
          ...
          sucesso: false,
          mensagem:
            error instanceof Error ? error.message : "Falha não identificada na sincronização.",
          payload_bruto: {
