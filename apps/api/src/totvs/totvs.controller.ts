...
export class TotvsController {
  ...
  private async scopedConnections(organizationId: string) {
    ...
    const hasLegacyDefault = (companies.data ?? []).some(
  ...
  async settings(@Req() request: AuthenticatedRequest) {
    ...
    const [
      ...
      supabaseAdmin
        ...
        .select(
          "id, connection_key, direction, entity, status, trigger, started_at, finished_at, metrics, error_message, created_at",
        )
        .eq("organization_id", organizationId)
        .order("created_at", { ascending: false })
        .limit(50),
      supabaseAdmin
        .from("totvs_sync_checkpoints")
        .select(
          "connection_key, entity, last_attempt_at, last_success_at, source_watermark, rows_processed, last_error",
        )
        .eq("organization_id", organizationId)
        .order("entity"),
      supabaseAdmin
        ...
        .select(
          "id, fiscal_document_id, status, attempt, rm_record_id, error_message, started_at, finished_at, created_at",
        )
        .eq("organization_id", organizationId)
        .order("created_at", { ascending: false })
        .limit(50),
      ...
      supabaseAdmin
        ...
        .select("id, company_id, acao, sucesso, mensagem, payload_bruto, created_at")
        .eq("organization_id", organizationId)
        .order("created_at", { ascending: false })
        .limit(100),
    ...
    const fiscalStates = companyIds.length
      ? await supabaseAdmin
          .from("fiscal_distribution_state")
          .select("company_id, last_sync_at, next_allowed_sync_at, last_cstat, last_error")
          .in("company_id", companyIds)
      : { data: [], error: null };
    if (fiscalStates.error) throw fiscalStates.error;
    const accountPlan = await this.plans.account(organizationId);
    ...
    return {
      ...
      fiscalStates: fiscalStates.data ?? [],
      accountPlan,
      ...
      environment: {
        ...
        coligadas:
          scopedConnections.find((connection) => connection.key === this.sqlServer.defaultKey())
            ?.coligadas ?? [],
  ...
  async saveSettings(@Body() body: unknown, @Req() request: AuthenticatedRequest) {
    ...
    if (input.nfeSchedules.some((schedule) => schedule.enabled))
      await this.plans.assertFeature(
        organizationId,
        "automatic_nfe",
        "Sincronização automática de NF-e",
      );
    if (input.nfseSchedules.some((schedule) => schedule.enabled))
      await this.plans.assertFeature(
        organizationId,
        "automatic_nfse",
    ...
    const companyIds = [
      ...new Set([
        ...input.companyMappings.map((mapping) => mapping.companyId),
        ...input.nfeSchedules.map((schedule) => schedule.companyId),
        ...input.nfseSchedules.map((schedule) => schedule.companyId),
      ]),
    ];
    if (companyIds.length > 0) {
      ...
      if ((allowed.data?.length ?? 0) !== companyIds.length)
        throw new BadRequestException("Uma empresa informada não pertence à organização.");
    }
  ...
  async testConnection(@Body() body: unknown, @Req() request: AuthenticatedRequest) {
    ...
    const { connectionKey } = z
      .object({ connectionKey: connectionKeySchema.optional() })
      .parse(body ?? {});
  ...
  async sync(@Body() body: unknown, @Req() request: AuthenticatedRequest) {
    ...
    const linked = await supabaseAdmin
      ...
      .eq("organization_id", organizationId)
      .or(
        `totvs_connection_key.eq.${key},and(totvs_connection_key.is.null,totvs_coligada_id.not.is.null)`,
      )
      .limit(1)
      .maybeSingle();
  ...
  async integrate(@Param("documentId") documentId: string, @Req() request: AuthenticatedRequest) {
    ...
    const company = await supabaseAdmin
      .from("companies")
      .select("organization_id")
      .eq("id", document.data.company_id)
      .single();
    if (company.error || company.data.organization_id !== organizationId)
