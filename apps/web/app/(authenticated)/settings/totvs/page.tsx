...
export default function TotvsSettingsPage() {
  ...
  return <div className="space-y-6">
    ...
    <Tabs defaultValue="config"><TabsList className="flex h-auto flex-wrap"><TabsTrigger value="config">TOTVS RM</TabsTrigger><TabsTrigger value="nfe">NF-e automática</TabsTrigger><TabsTrigger value="checkpoints">Checkpoints</TabsTrigger><TabsTrigger value="runs">Logs</TabsTrigger><TabsTrigger value="integrations">NF-e → RM</TabsTrigger></TabsList>
      ...
      <TabsContent value="nfe" className="mt-4"><Card><CardHeader><CardTitle className="text-base">Recorrência da consulta NF-e</CardTitle><CardDescription>O BullMQ consulta cada empresa no intervalo definido, respeitando o cooldown da SEFAZ e impedindo execuções simultâneas.</CardDescription></CardHeader><CardContent><div className="grid gap-3 md:grid-cols-2">{query.data.companies.map((company) => { const schedule = form.nfeSchedules.find((item) => item.companyId === company.id); return <div key={company.id} className="space-y-4 rounded-lg border p-4"><div className="flex items-center justify-between gap-4"><div className="min-w-0"><p className="truncate text-sm font-medium">{company.nome_fantasia || company.razao_social}</p><p className="font-mono text-xs text-slate-500">{company.cnpj}</p></div><Switch checked={schedule?.enabled ?? false} onCheckedChange={(enabled) => setForm({ ...form, nfeSchedules: form.nfeSchedules.map((item) => item.companyId === company.id ? { ...item, enabled } : item) })} /></div><div className="space-y-2"><Label>Intervalo entre consultas (minutos)</Label><Input type="number" min={15} max={1440} value={schedule?.intervalMinutes ?? 60} onChange={(event) => setForm({ ...form, nfeSchedules: form.nfeSchedules.map((item) => item.companyId === company.id ? { ...item, intervalMinutes: Number(event.target.value) } : item) })} /></div></div>; })}</div><div className="mt-5 flex justify-end"><Button onClick={() => save.mutate()} disabled={save.isPending}>{save.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Salvar recorrências</Button></div></CardContent></Card></TabsContent>
