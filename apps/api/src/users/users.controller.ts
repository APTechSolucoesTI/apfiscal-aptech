...
export class UsersController {
  ...
  async list(@Req() request: AuthenticatedRequest, @Headers("x-organization-id") orgHeader?: string) {
    ...
    const [members, profiles, companies] = await Promise.all([
      supabaseAdmin.from("organization_members").select("id, user_id, profile_id, role, active, created_at").eq("organization_id", organizationId),
      supabaseAdmin.from("access_profiles").select("id, name, active").eq("organization_id", organizationId).eq("active", true).order("name"),
      supabaseAdmin.from("companies").select("id, razao_social, nome_fantasia").eq("organization_id", organizationId).order("razao_social"),
    ]);
    if (members.error) throw members.error;
    const userIds = (members.data ?? []).map((member) => member.user_id);
    const [users, access] = userIds.length ? await Promise.all([
    ...
    const profileMap = new Map((profiles.data ?? []).map((profile) => [profile.id, profile.name] as const));
    ...
    return {
      users: (members.data ?? []).map((member) => ({
        ...
        company_ids: (access.data ?? []).filter((item) => item.user_id === member.user_id).map((item) => item.company_id),
      })),
  ...
  @Post("invite")
  async invite(@Body() raw: unknown, @Req() request: AuthenticatedRequest, @Headers("x-organization-id") orgHeader?: string) {
  ...
  @RequirePermission("settings.users.manage")
  @Patch(":id")
  async update(@Param("id") userId: string, @Body() raw: unknown, @Req() request: AuthenticatedRequest, @Headers("x-organization-id") orgHeader?: string) {
    const input = memberSchema.omit({ email: true }).extend({ active: z.boolean() }).parse(raw);
    const organizationId = await this.organizationId(request, orgHeader);
    const member = await supabaseAdmin.from("organization_members").update({ profile_id: input.profileId, active: input.active }).eq("organization_id", organizationId).eq("user_id", userId);
    if (member.error) throw member.error;
    const user = await supabaseAdmin.from("users").update({ full_name: input.fullName, active: input.active }).eq("id", userId);
    if (user.error) throw user.error;
