...
import { NfeCobrancaEditor } from "./NfeCobrancaEditor";
import {
  statusConfig,
...
import { unlinkNfeItem, linkNfeItemToProduct } from "@/lib/client-actions";
...
const tpNfLabel = (v: unknown) =>
  String(v) === "0" ? "0 - Entrada" : String(v) === "1" ? "1 - Saída" : String(v ?? "-");
const finLabel = (v: unknown) => {
  const s = String(v ?? "");
  return (
    { "1": "1 - Normal", "2": "2 - Complementar", "3": "3 - Ajuste", "4": "4 - Devolução" }[s] ??
    (s || "-")
  );
};
...
export const NfeDetalhes = ({ nfeId }: { nfeId: string }) => {
  ...
  const linkSugMut = useMutation({
    mutationFn: (v: { itemId: string; produtoId: string }) =>
      linkFn({ data: { itemId: v.itemId, produtoId: v.produtoId } }),
    onSuccess: () => {
  ...
  const sugestoes = ((data as any).suggestions ?? {}) as Record<
    string,
    {
      produto_id: string;
  ...
  const emitEndereco = [
    enderEmit.xLgr ?? enderEmit.logradouro,
    enderEmit.nro ?? enderEmit.numero,
    enderEmit.xBairro ?? enderEmit.bairro,
    enderEmit.xMun ?? enderEmit.municipio,
    enderEmit.UF ?? enderEmit.uf,
    emitCep ? `CEP ${emitCep}` : null,
  ]
    .filter(Boolean)
    .join(", ");
  const destEndereco = [
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <Badge variant="secondary" className={`border ${st.badge}`}>
                {st.label}
              </Badge>
      ...
      >
        <DialogContent className="max-w-5xl w-[95vw] h-[90vh] flex flex-col p-0 gap-0">
          ...
          <DialogFooter className="p-4 border-t">
            <Button variant="outline" onClick={() => setDanfePreview(null)}>
              Fechar
            </Button>
            <Button
              onClick={() => {
          ...
          <div className="space-y-6 pb-10">
                ...
                <Card>
                  <CardContent className="p-4">
                    ...
                    <p className="text-2xl font-bold text-destructive">
                      {fmt(doc.valor_desconto ?? totais.vDesc)}
                    </p>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="p-4">
                    ...
                    <p className="text-2xl font-bold text-amber-600">
                      {fmt(doc.valor_impostos ?? totais.vTotTrib)}
                    </p>
                  </CardContent>
                </Card>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                  ...
                  <CardContent className="grid grid-cols-2 gap-y-4 text-sm">
                    <div>
                      ...
                      <p className="font-medium">{doc.natureza_operacao ?? "-"}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">Tipo de Operação</p>
                      <p className="font-medium">{tpNfLabel(doc.tipo_operacao)}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">Finalidade</p>
                      <p className="font-medium">{finLabel(doc.finalidade)}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">Protocolo</p>
                      <p className="font-medium">{doc.protocolo ?? "-"}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">Data Emissão</p>
                      <p className="font-medium">{dt(doc.data_emissao)}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">Data Autorização</p>
                      <p className="font-medium">{dt(doc.data_autorizacao)}</p>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3 text-sm">
                    <div className="flex justify-between font-bold gap-4">
                      <span>
                        {dest.xNome ?? doc.destinatario_nome ?? doc.companies?.razao_social ?? "-"}
                      </span>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <p>
                        <span className="text-muted-foreground">IE:</span> {dest.IE ?? "-"}
                      </p>
                      <p>
                        <span className="text-muted-foreground">Ind. IE Dest.:</span>{" "}
                        {dest.indIEDest ?? "-"}
                      </p>
                    </div>
                  </CardContent>
                      ...
                      <tbody>
                        ...
                        {items.map((item) => {
                          ...
                          return (
                            <tr
                              key={item.id}
                              className={`border-b hover:bg-muted/50 cursor-pointer transition-colors ${pendente ? "bg-amber-50/40" : ""}`}
                            ...
                            >
                                ...
                                {!pendente && produto ? (
                                  ...
                                  <div className="rounded border border-blue-200 bg-blue-50 p-2 text-xs space-y-1">
                                    ...
                                    <p className="font-mono text-blue-900">
                                      {sugestao.codigo_interno}
                                    </p>
                                    <p className="font-medium text-foreground">
                                      {sugestao.descricao}
                                    </p>
                                    <p className="text-muted-foreground">
                                      UN {sugestao.unidade ?? "-"} · NCM {sugestao.ncm ?? "-"}
                                    </p>
                                    <Button
                                      size="sm"
                                      variant="outline"
                                      className="h-7 mt-1 border-blue-300 text-blue-800 hover:bg-blue-100"
                              ...
                              <td className="px-4 py-4 text-right">
                                {Number(item.quantidade_comercial ?? 0).toLocaleString("pt-BR")}
                              </td>
                              <td className="px-4 py-4 text-right">
                                {fmt(item.valor_unitario_comercial)}
                              </td>
                                  ...
                                  {!pendente && (
                                    <Button
                                      ...
                                      onClick={(e) => {
                                        ...
                                        if (
                                          confirm(
                                            "Desvincular este item do produto? O item voltará para o status pendente.",
                                          )
                                        ) {
            ...
            <TabsContent value="financeiro" className="m-0">
              <NfeFinanceiro
                doc={doc}
                items={items}
                readOnly={!podeEditarApontamentos(doc.status)}
              />
            </TabsContent>
            ...
            <TabsContent value="impostos" className="m-0 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card>
                  <CardHeader className="py-3">
                    <CardTitle className="text-sm uppercase text-muted-foreground">ICMS</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="flex justify-between items-end">
                      <div>
                        <p className="text-xs text-muted-foreground">BC ICMS</p>
                        <p className="font-bold">{fmt(totais.vBC)}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-xs text-muted-foreground">Valor ICMS</p>
                        <p className="text-xl font-bold text-primary">{fmt(totais.vICMS)}</p>
                      </div>
                    </div>
                    <div className="flex justify-between items-end mt-3">
                      <div>
                        <p className="text-xs text-muted-foreground">BC ICMS ST</p>
                        <p>{fmt(totais.vBCST)}</p>
                      </div>
                      <div className="text-right">
                ...
                <Card>
                  <CardHeader className="py-3">
                    <CardTitle className="text-sm uppercase text-muted-foreground">IPI</CardTitle>
                  </CardHeader>
                  <CardContent>
                ...
                <Card>
                  ...
                  <CardContent className="flex justify-between">
                    <div>
                      ...
                      <p className="font-bold">{fmt(totais.vPIS)}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-xs text-muted-foreground">COFINS</p>
                      <p className="font-bold">{fmt(totais.vCOFINS)}</p>
                    </div>
                  </CardContent>
                </Card>
              </div>
              ...
              {ibscbsTotais.present ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <Card>
                    <CardHeader className="py-3">
                      <CardTitle className="text-sm uppercase text-muted-foreground">IBS</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <div className="flex justify-between items-end">
                        <div>
                          <p className="text-xs text-muted-foreground">Base IBS/CBS</p>
                          <p className="font-bold">{fmt(ibscbsTotais.vBC)}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-xs text-muted-foreground">Total IBS</p>
                          <p className="text-xl font-bold text-primary">{fmt(ibscbsTotais.vIBS)}</p>
                        </div>
                      </div>
                      <div className="flex justify-between items-end mt-3">
                        <div>
                          <p className="text-xs text-muted-foreground">IBS UF</p>
                          <p>{fmt(ibscbsTotais.vIBSUF)}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-xs text-muted-foreground">IBS Município</p>
                          <p>{fmt(ibscbsTotais.vIBSMun)}</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                  <Card>
                    <CardHeader className="py-3">
                      <CardTitle className="text-sm uppercase text-muted-foreground">CBS</CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                <p className="text-sm text-muted-foreground">
                  Esta NF-e não possui tributos da Reforma Tributária (IBS / CBS / IS) informados no
                  XML.
                </p>
              )}
            </TabsContent>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <p>
                          {doc_(transp.transporta?.CNPJ ?? transp.transporta?.CPF)} | IE:{" "}
                          {transp.transporta?.IE ?? "-"}
                        </p>
                        <p className="text-muted-foreground text-xs">
                          {transp.transporta?.xEnder ?? ""} {transp.transporta?.xMun ?? ""}{" "}
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          <p className="text-muted-foreground">Placa / UF</p>
                        ...
                        <div>
                          ...
                          <p className="font-medium">{vol.pesoL ?? "-"} Kg</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Peso Bruto</p>
                          <p className="font-medium">{vol.pesoB ?? "-"} Kg</p>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <TabsContent value="cobranca" className="m-0 space-y-6">
              <Card>
                <CardHeader className="flex flex-row items-center justify-between gap-3">
                  ...
                  <NfeCobrancaEditor
                    documentId={doc.id}
                    total={Number(doc.valor_total ?? 0)}
                    disabled={!podeEditarApontamentos(doc.status)}
                  />
                </CardHeader>
                <CardContent className="space-y-6">
                  {cobr?.fat && (
                    <div className="bg-muted/50 p-4 rounded-lg grid grid-cols-3 gap-4 text-sm">
                      <div>
                        <p className="text-muted-foreground">Número Fatura</p>
                        <p className="font-bold">{cobr.fat.nFat ?? "-"}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Valor Original</p>
                        <p className="font-bold">{fmt(cobr.fat.vOrig)}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Valor Líquido</p>
                        <p className="font-bold text-primary text-lg">{fmt(cobr.fat.vLiq)}</p>
                      </div>
                    </div>
                  )}
                  {dupl.length > 0 ? (
                  ...
                  {events.length === 0 ? (
                    ...
                    <div className="space-y-4">
                      {events.map((ev: any) => (
                        <div
                          key={ev.id}
                          className="flex gap-3 p-4 rounded border border-muted bg-white shadow-sm"
                        >
