...
export const aprovarNfe = createServerFn({ method: "POST" })
...
export const reavaliarStatusApontamentos = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
...
export const marcarIntegradoTotvs = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
...
export const listStatusHistorico = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .inputValidator((data: { documentId: string }) => data)
  .handler(async ({ data, context }) => {
