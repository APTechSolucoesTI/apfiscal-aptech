...
const MUTATING_SQL =
  /\b(?:INSERT|UPDATE|DELETE|MERGE|TRUNCATE|DROP|ALTER|CREATE|EXEC(?:UTE)?|GRANT|REVOKE|DENY|BACKUP|RESTORE)\b/i;
const KEY_PATTERN = /^[A-Z][A-Z0-9_]{1,63}$/;

export type TotvsConnectionSummary = {
  key: string;
  description: string;
...
type TotvsConnectionConfig = TotvsConnectionSummary & {
  host: string | null;
  port: number;
  user: string | null;
...
export function assertTotvsWriteDatabase(
  database: string | null,
  configuredDatabase: string | null,
): asserts database is string {
  if (!configuredDatabase || database !== configuredDatabase)
    throw new Error(
      `Escrita recusada: o banco conectado (${database ?? "não informado"}) não corresponde ao banco configurado para esta conexão (${configuredDatabase ?? "não informado"}).`,
    );
}

const flag = (value: string | undefined, fallback: boolean) =>
  value?.trim() ? ["1", "true", "yes", "on"].includes(value.trim().toLowerCase()) : fallback;
const coligadas = (value: string | undefined) => [
  ...new Set(
    (value ?? "1,2")
      .split(",")
...
export class TotvsSqlServerService implements OnModuleDestroy {
  ...
  defaultKey() {
    return normalizeKey(
      process.env.TOTVS_DEFAULT_CONNECTION_KEY ?? process.env.TOTVS_CONNECTION_KEY,
    );
  }

  connectionKeys() {
    const keys = (process.env.TOTVS_CONNECTION_KEYS ?? "")
      .split(",")
      .map((item) => item.trim())
      .filter(Boolean)
  ...
  connections(): TotvsConnectionSummary[] {
    return this.connectionKeys().map((key) => {
      ...
      return {
        ...
        coligadas: config.coligadas,
      };
    });
  }

  configured(key = this.defaultKey()) {
    return this.config(key).configured;
  }
  writesEnabled(key = this.defaultKey()) {
    return this.config(key).writesEnabled;
  }
  coligadas(key = this.defaultKey()) {
    const values = this.config(key).coligadas;
    if (!values.length)
      throw new Error(`A conexão ${key} deve permitir ao menos uma coligada numérica.`);
    return values;
  }

  private legacyConfigured() {
    return ["TOTVS_SQL_HOST", "TOTVS_SQL_DATABASE", "TOTVS_SQL_USER", "TOTVS_SQL_PASSWORD"].every(
  ...
  private config(keyInput: string): TotvsConnectionConfig {
    ...
    const get = (suffix: string, legacy?: string) =>
      process.env[`${prefix}${suffix}`]?.trim() ||
      (isDefault && legacy ? process.env[legacy]?.trim() : undefined);
    ...
    return {
      ...
      trustServerCertificate: flag(
        get("TRUST_SERVER_CERTIFICATE", "TOTVS_SQL_TRUST_SERVER_CERTIFICATE"),
        false,
      ),
      connectionTimeout: Number(
        get("CONNECTION_TIMEOUT_MS", "TOTVS_SQL_CONNECTION_TIMEOUT_MS") || 15_000,
      ),
  ...
  private pool(keyInput: string): Promise<sql.ConnectionPool> {
    ...
    if (!config.configured || !config.host || !config.database || !config.user || !config.password)
      throw new Error(`Credenciais ausentes para a conexão TOTVS ${config.key}.`);
  ...
  async testConnection(
    key = this.defaultKey(),
  ): Promise<{ ok: true; key: string; description: string; database: string }> {
    const result = await this.queryReadOnly<{ ok: number; database_name: string }>(
      "SELECT 1 AS ok, DB_NAME() AS database_name",
      {},
      key,
    );
    if (result[0]?.ok !== 1)
      throw new Error("O SQL Server respondeu de forma inesperada ao SELECT 1.");
    return {
      ok: true,
      key,
      description: this.config(key).description,
      database: result[0].database_name,
    };
  }

  async queryReadOnly<T extends Record<string, unknown>>(
    statement: string,
  ...
  assertWritesEnabled(key = this.defaultKey()) {
    ...
    if (!config.writesEnabled)
      throw new Error(`Escritas no TOTVS RM estão bloqueadas para ${key}.`);
    assertTotvsWriteDatabase(config.database, config.database);
  }
  ...
  async writeTransaction<T>(
    ...
    try {
      ...
      assertTotvsWriteDatabase(
        target.recordset[0]?.database_name ?? null,
        this.config(key).database,
      );
