...
export function getTotvsSettings() {
  return backendFetch<TotvsSettingsPayload>("/totvs/settings");
}
...
export function saveTotvsSettings(input: {
  ...
  return backendFetch<{ ok: true }>("/totvs/settings", { method: "PATCH", body: JSON.stringify(input) });
}

export function testTotvsConnection() {
  return backendFetch<{ ok: true; database: string }>("/totvs/test-connection", { method: "POST" });
}

export function enqueueTotvsSync() {
  return backendFetch<{ runId: string; status: string; idempotent: boolean }>("/totvs/sync", { method: "POST" });
}
