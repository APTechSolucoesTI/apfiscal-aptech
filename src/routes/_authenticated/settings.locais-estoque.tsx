...
import {
  listLocaisEstoque, saveLocalEstoque, toggleLocalEstoqueAtivo, deleteLocalEstoque,
  type LocalEstoqueInput,
} from "@/lib/locais-estoque.functions";
...
import { Plus, Pencil, Trash2, Search, Loader2, Warehouse, Upload, ChevronRight, ChevronDown, FolderTree, FileText } from "lucide-react";
import { toast } from "sonner";
import { ImportXlsxDialog, type ImportField } from "@/components/import/ImportXlsxDialog";

const leImportFields: ImportField[] = [
  { key: "codigo", label: "Código", required: true, aliases: ["codigo", "cod", "codigolocal", "local"] },
...
export const Route = createFileRoute("/_authenticated/settings/locais-estoque")({
  component: LocaisEstoquePage,
  head: () => ({
    meta: [
      { title: "Locais de Estoque | APFiscal" },
      { name: "description", content: "Cadastro hierárquico de Locais de Estoque (sintético 99 e analítico 99.999)." },
      { property: "og:title", content: "Locais de Estoque | APFiscal" },
      { property: "og:description", content: "Gerencie os Locais de Estoque da sua operação." },
...
function LocaisEstoquePage() {
  ...
  const [form, setForm] = useState<LocalEstoqueInput>({ company_id: null, codigo: "", descricao: "", ativo: true });
  ...
  const listFn = useServerFn(listLocaisEstoque);
  const saveFn = useServerFn(saveLocalEstoque);
  const toggleFn = useServerFn(toggleLocalEstoqueAtivo);
  const delFn = useServerFn(deleteLocalEstoque);

  const isGlobalView = companyId === GLOBAL;

  const { data: rows = [], isLoading } = useQuery({
    queryKey: ["locais-estoque", companyId],
    enabled: !!companyId,
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => {
      toast.success(form.id ? "Local de Estoque atualizado" : "Local de Estoque criado");
      qc.invalidateQueries({ queryKey: ["locais-estoque"] });
      setOpen(false);
    },
  ...
  const toggleMut = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ["locais-estoque"] }),
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  // Sugere o próximo código disponível (raiz 99 ou filho 99.999)
      ...
      <div key={n.row.id}>
        <div className="flex items-center gap-2 px-3 py-2 border-b hover:bg-slate-50" style={{ paddingLeft: `${12 + depth * 20}px` }}>
          ...
          <div className="flex items-center gap-1">
            {!analitico && (
              <Button size="sm" variant="ghost" onClick={() => openNew(n.row)} title="Adicionar local analítico">
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={() => openNew(null)} disabled={!companyId}><Plus className="h-4 w-4 mr-1" /> Novo Local de Estoque</Button>
            </div>
          </div>
        </CardHeader>
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="text-center py-12 text-slate-500">Nenhum local de estoque cadastrado.</div>
          ) : (
      ...
      <ImportXlsxDialog
        ...
        title="Importar Locais de Estoque via Excel"
        ...
        buildRow={(m, ctx) => {
          ...
          const codigo = normalizeCodigoLE(m.codigo);
          const descricao = String(m.descricao ?? "").trim();
          if (!descricao) throw new Error("Descrição obrigatória");
          return { company_id: ctx.companyId ?? null, codigo, descricao, ativo: parseAtivo(m.ativo) } as LocalEstoqueInput;
        }}
        checkDuplicate={async (row) => {
          let q = supabase
            .from("locais_estoque" as any)
        ...
        onDone={() => qc.invalidateQueries({ queryKey: ["locais-estoque"] })}
      />

      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{form.id ? "Editar" : "Novo"} Local de Estoque</DialogTitle>
            <DialogDescription>
              {nivelDoCodigo(form.codigo || "") === 2
                ? "Local analítico — o prefixo do código herda do local sintético."
                : "Local sintético — nível 1 (99)."}
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3">
            ...
            <div>
              ...
              <Input
                value={form.codigo}
                onChange={(e) => setForm({ ...form, codigo: maskCodigoLE(e.target.value) })}
                placeholder="01.001"
                maxLength={6}
                disabled={!!form.id}
                className="font-mono"
      ...
      <AlertDialog open={!!confirmInativar} onOpenChange={(o) => !o && setConfirmInativar(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Inativar Local de Estoque?</AlertDialogTitle>
            <AlertDialogDescription>
              Ao inativar, este Local de Estoque deixará de aparecer como opção em novos lançamentos.
      ...
      <AlertDialog open={!!confirmExcluir} onOpenChange={(o) => !o && setConfirmExcluir(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta ação é irreversível. Locais sintéticos com filhos vinculados não podem ser excluídos — nesse caso, inative o registro.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
