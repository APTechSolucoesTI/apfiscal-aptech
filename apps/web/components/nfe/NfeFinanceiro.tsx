...
import { listLocaisEstoque } from "@/lib/client-actions";
import { listTiposCompra } from "@/lib/client-actions";
import {
  getAlocacaoNfe,
  ...
  setAlocacoesItem,
  setLocalEstoqueCabecalho,
  setLocalEstoqueItem,
  setTipoCompraCabecalho,
  setTipoCompraItem,
  ...
  return (
    <div className="space-y-1 max-w-xl">
      <div className="flex items-center justify-between text-xs text-slate-600">
        ...
        <b>
          {apontados}/{total} itens apontados
        </b>
      </div>
      <Progress value={pct} className="h-2" />
      {faltantes > 0 && (
        <p className="text-xs font-medium text-amber-700">
          Existem {faltantes} item(ns) sem {faltanteLabel} apontado. Realize o apontamento antes de
          prosseguir.
        </p>
      )}
    </div>
...
export function NfeFinanceiro({
  ...
  useEffect(() => {
    if (!alocacao) return;
    setCabAllocs(
      (alocacao.cabecalho as any[]).map((r) => ({
  ...
  const setPCHeaderMut = useMutation({
    mutationFn: (v: { planoContasId: string | null; sobrescreverItens: boolean }) =>
      setPCHeaderFn({
        data: {
          documentId,
          planoContasId: v.planoContasId,
          sobrescreverItens: v.sobrescreverItens,
        },
      }),
    onSuccess: () => {
  ...
  const setLEHeaderMut = useMutation({
    mutationFn: (v: { localEstoqueId: string | null; sobrescreverItens: boolean }) =>
      setLEHeaderFn({
        data: {
          documentId,
          localEstoqueId: v.localEstoqueId,
          sobrescreverItens: v.sobrescreverItens,
        },
      }),
    onSuccess: () => {
  ...
  const setTCHeaderMut = useMutation({
    mutationFn: (v: { tipoCompraId: string | null; sobrescreverItens: boolean }) =>
      setTCHeaderFn({
        data: { documentId, tipoCompraId: v.tipoCompraId, sobrescreverItens: v.sobrescreverItens },
      }),
    onSuccess: () => {
  ...
  const saveCabMut = useMutation({
    mutationFn: (propagar: boolean) =>
      setCabFn({ data: { documentId, alocacoes: cabAllocs, propagarParaItens: propagar } }),
    onSuccess: () => {
  ...
  const saveItemMut = useMutation({
    mutationFn: (itemId: string) =>
      setItemFn({ data: { itemId, alocacoes: itemAllocs[itemId] ?? [] } }),
    onSuccess: () => {
  ...
  const consolidacaoTC = useMemo(
    () =>
      consolidarTipoCompra(
        items.map((it) => ({ id: it.id, tipo_compra_id: it.tipo_compra_id ?? null })),
      ),
    [items],
  );
  const progressoTC =
    consolidacaoTC.total > 0 ? (consolidacaoTC.apontados / consolidacaoTC.total) * 100 : 0;
  ...
  function addItemRow(itemId: string) {
    setItemAllocs((p) => ({
      ...p,
      [itemId]: [...(p[itemId] ?? []), { centro_custo_id: ccOptions[0]?.id ?? "", valor: 0 }],
    }));
  }

  const previewCab = useMemo(
    () =>
      recalcularAlocacaoCabecalho(
        items.map((it) => ({
          id: it.id,
          valor_bruto: Number(it.valor_bruto || 0),
          alocacoes: itemAllocs[it.id] ?? [],
        })),
      ),
    [items, itemAllocs],
  );
    ...
    <div className="space-y-6">
      {!readOnly && (
        <div className="flex flex-col gap-3 rounded-lg border border-blue-200 bg-blue-50/70 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="font-semibold text-blue-950">Preencher a partir do histórico</p>
            <p className="text-sm text-blue-800">
              Usa notas já integradas do mesmo fornecedor e produtos semelhantes. Você pode ajustar
              tudo depois.
            </p>
          </div>
          <Button
      ...
      <fieldset disabled={readOnly} className={readOnly ? "space-y-6 opacity-60" : "space-y-6"}>
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <ProgressoApontamento
              ...
              total={totalItens}
              faltanteLabel="Plano de Contas"
            />
            <p className="text-xs text-slate-500">
              Ao alterar, o Plano de Contas é propagado para todos os itens (exceto os alterados
              manualmente).
            </p>
          </CardContent>
        </Card>
              ...
              {consolidacaoTC.multiplos ? (
                <Popover>
                  <PopoverTrigger asChild>
                    <Badge
                      variant="outline"
                      className="cursor-pointer bg-violet-50 text-violet-700 border-violet-200"
                    >
                      Múltiplos tipos
                    </Badge>
                  </PopoverTrigger>
                    ...
                    <ul className="text-sm space-y-1">
                      {consolidacaoTC.distribuicao.map((d) => (
                        <li key={d.tipo_compra_id ?? "__none__"}>
                          ...
                          {d.tipo_compra_id
                            ? labelTipoCompra(tcById.get(d.tipo_compra_id))
                            : "sem Tipo de Compra"}
                        </li>
                      ))}
                    </ul>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="space-y-1 max-w-xl">
              <div className="flex items-center justify-between text-xs text-slate-600">
                <span>Progresso do apontamento</span>
                <b>
                  {consolidacaoTC.apontados}/{consolidacaoTC.total} itens apontados
                </b>
              </div>
            ...
            {consolidacaoTC.faltantes > 0 && (
              <p className="text-xs font-medium text-amber-700">
                Existem {consolidacaoTC.faltantes} item(ns) sem Tipo de Compra apontado. Realize o
                apontamento antes de prosseguir.
              </p>
            )}
            <p className="text-xs text-slate-500">
              Ao alterar, o Tipo de Compra é propagado para todos os itens (exceto os alterados
              manualmente).
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <ProgressoApontamento
              ...
              total={totalItens}
              faltanteLabel="Local de Estoque"
            />
            <p className="text-xs text-slate-500">
              Somente locais analíticos (99.999) podem ser usados. Ao alterar, o Local de Estoque é
              propagado para todos os itens (exceto os alterados manualmente).
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-lg">Rateio de Centros de Custo — Cabeçalho</CardTitle>
            </div>
            <div className="flex items-center gap-4">
              ...
              <div className="text-sm">
                ...
                <span className="text-slate-500">
                  ({pctDe(totalAlocadoCab, valorTotal).toFixed(2)}%)
                </span>{" "}
                {" / "}
                <span className="text-slate-500">Total NF-e:</span> <b>{fmt(valorTotal)}</b> {" / "}
                <span className="text-slate-500">Restante:</span>{" "}
                <b className={restante < 0 ? "text-red-600" : ""}>{fmt(restante)}</b>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-2">
            <ProgressoApontamento
              apontados={apontadosCC}
              total={totalItens}
              faltanteLabel="rateio de Centro de Custo"
            />
            {cabAllocs.map((a, idx) => (
              <div key={idx} className="flex items-center gap-2">
                ...
                <div className="relative w-40">
                  <Input
                    ...
                    value={
                      modoRateio === "percentual" ? round2(pctDe(a.valor, valorTotal)) : a.valor
                    }
                    onChange={(e) => {
                      const raw = Number(e.target.value || 0);
                      const v =
                        modoRateio === "percentual" ? round2((raw / 100) * valorTotal) : raw;
                ...
                <span className="w-28 text-right text-xs text-slate-500">
                  {modoRateio === "percentual"
                    ? fmt(a.valor)
                    : `${round2(pctDe(a.valor, valorTotal)).toFixed(2)}%`}
                </span>
                <Button
                  size="icon"
                  variant="ghost"
                  onClick={() => setCabAllocs((p) => p.filter((_, i) => i !== idx))}
                >
            ...
            <div className="flex gap-2 pt-2">
              <Button variant="outline" size="sm" onClick={addCabRow}>
                <Plus className="h-4 w-4 mr-1" /> Adicionar Centro de Custo
              </Button>
              <Button
                size="sm"
                onClick={() => saveCabMut.mutate(false)}
                disabled={saveCabMut.isPending || totalAlocadoCab > valorTotal + 0.005}
              >
                {saveCabMut.isPending ? (
              ...
              <Button
                size="sm"
                variant="secondary"
                onClick={() => saveCabMut.mutate(true)}
                disabled={saveCabMut.isPending || totalAlocadoCab > valorTotal + 0.005}
              >
          ...
          <CardContent className="space-y-6">
                ...
                <div key={it.id} className="border rounded-md p-3 space-y-2">
                  <div className="flex items-center justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        Item: <b>{fmt(vTot)}</b> · Alocado: <b>{fmt(somaItem)}</b> · Disponível:{" "}
                        <b className={disponivel < 0 ? "text-red-600" : ""}>{fmt(disponivel)}</b>
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-64">
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button
                                  ...
                                  variant="ghost"
                                  className="h-8 w-8"
                                  onClick={() => restaurarTCMut.mutate(it.id)}
                                >
                    ...
                    <div key={idx} className="flex items-center gap-2">
                      <Select
                        ...
                        onValueChange={(v) =>
                          setItemAllocs((p) => ({
                            ...p,
                            [it.id]: (p[it.id] ?? []).map((x, i) =>
                              i === idx ? { ...x, centro_custo_id: v } : x,
                            ),
                          }))
                        }
                      >
                      ...
                      <div className="relative w-40">
                        <Input
                          ...
                          className="text-right pr-8"
                          value={
                            modoRateio === "percentual" ? round2(pctDe(a.valor, vTot)) : a.valor
                          }
                          onChange={(e) => {
                            const raw = Number(e.target.value || 0);
                            let v = modoRateio === "percentual" ? round2((raw / 100) * vTot) : raw;
                            const outros = (itemAllocs[it.id] ?? []).reduce(
                              (s, x, i) => s + (i === idx ? 0 : Number(x.valor || 0)),
                              0,
                            );
                          ...
                          onBlur={() => {
                            if (somaItem < vTot - 0.005) {
                              toast.warning(
                                `Item "${it.descricao}" tem ${fmt(vTot - somaItem)} do valor total ainda sem Centro de Custo alocado.`,
                              );
                            }
                          }}
                        />
                      ...
                      <span className="w-28 text-right text-xs text-slate-500">
                        {modoRateio === "percentual"
                          ? fmt(a.valor)
                          : `${round2(pctDe(a.valor, vTot)).toFixed(2)}%`}
                      </span>
                      ...
                      <Button
                        ...
                        onClick={() =>
                          setItemAllocs((p) => ({
                            ...p,
                            [it.id]: (p[it.id] ?? []).filter((_, i) => i !== idx),
                          }))
                        }
                      >
                        <Trash2 className="h-4 w-4 text-red-600" />
                  ...
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" onClick={() => addItemRow(it.id)}>
                      <Plus className="h-4 w-4 mr-1" /> Adicionar Centro
                    </Button>
                    <Button
                      size="sm"
                      onClick={() => saveItemMut.mutate(it.id)}
                      disabled={saveItemMut.isPending || somaItem > vTot + 0.005}
                    >
            ...
            {previewCab.length > 0 && (
              <div className="rounded-md border bg-muted/30 p-3">
                <p className="text-xs font-semibold uppercase text-slate-500 mb-2">
                  Prévia consolidada do cabeçalho (baseada nos itens)
                </p>
                <ul className="text-sm space-y-1">
                  {previewCab.map((c) => {
                    const cc = ccOptions.find((x: any) => x.id === c.centro_custo_id);
                    return (
                      <li key={c.centro_custo_id}>
                        <b>{cc?.codigo ?? "?"}</b> {cc?.descricao ?? ""}: {fmt(c.valor)}
                      </li>
                    );
                  })}
                </ul>
        ...
        <AlertDialog
          open={confirmSobrescrever !== null}
          onOpenChange={(o) => !o && setConfirmSobrescrever(null)}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Sobrescrever alterações manuais?</AlertDialogTitle>
              <AlertDialogDescription>
                Isso substituirá o Plano de Contas já definido manualmente em algum item. Deseja
                continuar?
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                onClick={() => {
                  ...
                  setPCHeaderMut.mutate({
                    planoContasId: id === "" ? null : id,
                    sobrescreverItens: true,
                  });
                }}
              >
                Sobrescrever
        ...
        <AlertDialog
          open={confirmSobrescreverLE !== null}
          onOpenChange={(o) => !o && setConfirmSobrescreverLE(null)}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Sobrescrever alterações manuais?</AlertDialogTitle>
              <AlertDialogDescription>
                Isso substituirá o Local de Estoque já definido manualmente em algum item. Deseja
                continuar?
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                onClick={() => {
                  ...
                  setLEHeaderMut.mutate({
                    localEstoqueId: id === "" ? null : id,
                    sobrescreverItens: true,
                  });
                }}
              >
        ...
        <AlertDialog
          open={confirmSobrescreverTC !== null}
          onOpenChange={(o) => !o && setConfirmSobrescreverTC(null)}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Sobrescrever alterações manuais?</AlertDialogTitle>
              <AlertDialogDescription>
                Isso substituirá o Tipo de Compra já definido manualmente em algum item. Deseja
                continuar?
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                onClick={() => {
                  ...
                  setTCHeaderMut.mutate({
                    tipoCompraId: id === "" ? null : id,
                    sobrescreverItens: true,
                  });
                }}
              >
